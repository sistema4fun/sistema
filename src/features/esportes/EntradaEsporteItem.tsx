...
import { updateEntradaEsporte, deleteEntradaEsporte } from "@/app/dashboard/esportes/actions";
...
type Row = {
  ...
  resultado?: "pendente" | "lucro" | "perda";
};

export default function EntradaEsporteItem({ row }: { row: Row }) {
  ...
  const oddStr = row.odd ? String(row.odd) : "";
  ...
  const [mercado, setMercado] = useState(row.mercado ?? "");
  const [odd, setOdd] = useState(oddStr);
  ...
  const onSave = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      try {
        await updateEntradaEsporte(row.id, {
          descricao,
          mercado,
          odd,
          stakeBRL: valorBRL,
  ...
  const onDelete = async () => {
    ...
    startTransition(async () => {
      try {
        await deleteEntradaEsporte(row.id);
      ...
      {!isEditing ? (
        <>
          <div className="flex flex-wrap items-center gap-x-12 sm:gap-x-14 gap-y-2">
            ...
            <span className={`uppercase ${textUnified}`}>{row?.descricao ?? "-"}</span>
            <span className={`uppercase ${textUnified}`}>{row?.mercado ?? "-"}</span>
            {oddStr && <span className={`uppercase ${textUnified} text-neutral-400`}>ODD {oddStr}</span>}
